...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 7312
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 7313
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 17312
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 17313
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 7312
	%{_sbindir}/semanage port -d -p tcp 7313
	%{_sbindir}/semanage port -d -p tcp 17312
	%{_sbindir}/semanage port -d -p tcp 17313
	for selinuxvariant in %{selinux_variants}; do
